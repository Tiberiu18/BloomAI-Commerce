...
productRoute.get("/", asyncHandler(async(req, res) => {
    ...
    const count = await Product.countDocuments({...keyword, ...priceInterval, ...categoryFilter});
    const products = await Product.find({...keyword, ...priceInterval, ...categoryFilter}).limit(pageSize).skip(pageSize*(page-1)).sort({_id: -1});
